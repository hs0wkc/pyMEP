...
class MainWindow(Ctk.CTk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Ctk.CTkEntry(self, textvariable=self.tbEvaporationRate, width=80, justify='right', state=tk.DISABLED, border_color='green').grid(column=1, row=5, sticky=tk.W, padx=5, pady=3)
        # Ctk.CTkLabel(self, textvariable=self.tbEvaporationRate, width=80, fg_color="gray30", corner_radius=6).grid(row=5, column=1, padx=5, pady=3, sticky=tk.W)
        ...
        link.grid(column=2, row=5, sticky=tk.E, padx=15, pady=3)
...
if __name__ == "__main__":
    ...
    with open(CSV_STEAMTABLE) as f:
        reader = DictReader(f)
        for row in reader:
            steam_pressure[row['Pressure']] = row['hg(j)']
    water_temp = {}
