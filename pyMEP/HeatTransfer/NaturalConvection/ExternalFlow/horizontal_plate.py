...
class Plate:
    ...
    def __init__(
        ...
        """
        self.L = L.to('m')
        self.W = W.to('m') if W is not None else None
    ...
    def Nusselt(self) -> float:
        if self.configuration in ['upwards-heated', 'downwards-cooled']:
            return average_nusselt_number_upwards(self.Ra, self.Pr)
        else:
            return average_nusselt_number_downwards(self.Ra, self.Pr)

    def avg_heat_trf_coeff(self) -> Quantity:
        """Returns the average heat transfer coefficient between the plate and
        the fluid.
        """
        h_avg = self.Nusselt * self.fluid.k / self.L_char
