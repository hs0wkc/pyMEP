...
def declination(n: float | np.ndarray) -> float | np.ndarray:
    """Returns the declination angle (delta) of the sun for day of the year `n` in degrees."""
    ...
    return delta_    
    # ASHRAE Handbook-Fundamentals 2021 p14.9, Declination
    # approximate equation of Cooper (1969):
    # return np.radians(23.45) * np.sin(2 * np.pi * (284 + n) / 365)

def hour_angle(solar_time: float | np.ndarray, rad_unit:bool=True) -> float | np.ndarray:
    ...
    ASHRAE Handbook-Fundamentals 2021 p14.10, Sun Position.
    ...
    """
    m = 15 * (solar_time - 12)
    omega =  np.radians(m) if rad_unit else m
    return omega
...
def profile_angle(
    ...
    """
    alpha_p_ = np.arctan(np.tan(alpha_s) / np.cos(gamma_s - gamma))
    return alpha_p_
