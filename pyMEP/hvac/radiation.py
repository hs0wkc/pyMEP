...
from .time import *
from .geometry import declination, daylight_time

"""The solar flux outside the atmosphere at the mean earth-sun distance, known as solar constant.
The solar constant used in the analysis is 1367 W/m2. 
This is consistent with the World Radiation Center (WRC) solar constant.
"""
G_sc = 1367.0  # W / m ** 2
...
def air_mass(theta_z: float | np.ndarray) -> float | np.ndarray:
    """Close approximation for air Mass at sea level when zenith angle is between 0° to 70°.
...
def ext_irradiance_normal(n: float | np.ndarray) -> float | np.ndarray:
    ...
    ASHRAE Fundamentals 2021 p14.8 Solar Constant and Extraterrestrial Solar Radiation

    n : day of the year
    """
    # G_on = G_sc * (1.0 + 0.033 * np.cos(np.radians(360 * n / 365)))
    G_on = G_sc * (1.0 + 0.033 * np.cos(np.radians(360 * (n-3) / 365)))
    return G_on

def ext_irradiance_hor_surf(n: float | np.ndarray, theta_z: float | np.ndarray) -> float | np.ndarray:
